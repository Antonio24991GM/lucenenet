    ...
    public class SPIClassIterator<S> : IEnumerable<Type>
    {
        ...
        static SPIClassIterator()
        {
            ...
            foreach (var loadedAssembly in AppDomain.CurrentDomain.GetAssemblies())
            {
                try
                {
                    foreach (var type in loadedAssembly.ExportedTypes)
                    {
                        try
                        {
                            if (typeof(S).GetTypeInfo().IsSubclassOf(type) && !type.GetTypeInfo().IsAbstract 
                                && !type.GetTypeInfo().IsInterface && type.GetTypeInfo().DeclaredConstructors.FirstOrDefault()!= null)
                                types.Add(type);
                        }
                        catch
                        {
