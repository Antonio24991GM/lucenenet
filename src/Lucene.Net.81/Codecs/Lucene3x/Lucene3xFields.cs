    ...
    public class Lucene3xFields : FieldsProducer
    {
        ...
        private class PreTermsEnum : TermsEnum
        {
            ...
            internal virtual bool SeekToNonBMP(SegmentTermEnum te, BytesRef term, int pos)
            {
                ...
                if (DEBUG_SURROGATES)
                {
                    System.Diagnostics.Debug.WriteLine("      try seek term=" + UnicodeUtil.ToHexString(term.Utf8ToString()));
                }
                ...
                if (DEBUG_SURROGATES)
                {
                    System.Diagnostics.Debug.WriteLine("      got term=" + UnicodeUtil.ToHexString(t2.Text()));
                }
            ...
            internal virtual bool DoContinue()
            {
                if (DEBUG_SURROGATES)
                {
                    System.Diagnostics.Debug.WriteLine("  try cont");
                }
                ...
                while (downTo > limit)
                {
                    if (IsHighBMPChar(PrevTerm.Bytes, downTo))
                    {
                        if (DEBUG_SURROGATES)
                        {
                            System.Diagnostics.Debug.WriteLine("    found E pos=" + downTo + " vs len=" + PrevTerm.Length);
                        }
                        ...
                        else
                        {
                            if (DEBUG_SURROGATES)
                            {
                                System.Diagnostics.Debug.WriteLine("      no seek");
                            }
                        }
                    }
            ...
            internal virtual bool DoPop()
            {
                if (DEBUG_SURROGATES)
                {
                    System.Diagnostics.Debug.WriteLine("  try pop");
                }
                ...
                if (PrevTerm.Length > NewSuffixStart && IsNonBMPChar(PrevTerm.Bytes, NewSuffixStart) && IsHighBMPChar(ScratchTerm.Bytes, NewSuffixStart))
                {
                    ...
                    if (DEBUG_SURROGATES)
                    {
                        System.Diagnostics.Debug.WriteLine("    seek to term=" + UnicodeUtil.ToHexString(ScratchTerm.Utf8ToString()) + " " + ScratchTerm.ToString());
                    }
                    ...
                    if (t2 != null && t2.Field == InternedFieldName)
                    {
                        if (DEBUG_SURROGATES)
                        {
                            System.Diagnostics.Debug.WriteLine("      got term=" + UnicodeUtil.ToHexString(t2.Text()) + " " + t2.Bytes);
                        }
                    ...
                    else if (NewSuffixStart != 0 || ScratchTerm.Length != 0)
                    {
                        if (DEBUG_SURROGATES)
                        {
                            System.Diagnostics.Debug.WriteLine("      got term=null (or next field)");
                        }
                        NewSuffixStart = 0;
            ...
            internal virtual void SurrogateDance()
            {
                ...
                if (DEBUG_SURROGATES)
                {
                    System.Diagnostics.Debug.WriteLine("  dance");
                    System.Diagnostics.Debug.WriteLine("    prev=" + UnicodeUtil.ToHexString(PrevTerm.Utf8ToString()));
                    System.Diagnostics.Debug.WriteLine("         " + PrevTerm.ToString());
                    System.Diagnostics.Debug.WriteLine("    term=" + UnicodeUtil.ToHexString(ScratchTerm.Utf8ToString()));
                    System.Diagnostics.Debug.WriteLine("         " + ScratchTerm.ToString());
                }
                ...
                if (DEBUG_SURROGATES)
                {
                    System.Diagnostics.Debug.WriteLine("  finish bmp ends");
                }
            ...
            internal virtual void DoPushes()
            {
                ...
                if (DEBUG_SURROGATES)
                {
                    System.Diagnostics.Debug.WriteLine("  try push newSuffixStart=" + NewSuffixStart + " scratchLen=" + ScratchTerm.Length);
                }
                ...
                while (upTo < ScratchTerm.Length)
                {
                    if (IsNonBMPChar(ScratchTerm.Bytes, upTo) && (upTo > NewSuffixStart || (upTo >= PrevTerm.Length || (!IsNonBMPChar(PrevTerm.Bytes, upTo) && !IsHighBMPChar(PrevTerm.Bytes, upTo)))))
                    {
                        ...
                        if (DEBUG_SURROGATES)
                        {
                            System.Diagnostics.Debug.WriteLine("    try seek 1 pos=" + upTo + " term=" + UnicodeUtil.ToHexString(ScratchTerm.Utf8ToString()) + " " + ScratchTerm.ToString() + " len=" + ScratchTerm.Length);
                        }
                        ...
                        if (DEBUG_SURROGATES)
                        {
                            if (t2 == null)
                            {
                                System.Diagnostics.Debug.WriteLine("      hit term=null");
                            }
                            else
                            {
                                System.Diagnostics.Debug.WriteLine("      hit term=" + UnicodeUtil.ToHexString(t2.Text()) + " " + (t2 == null ? null : t2.Bytes));
                            }
                        }
            ...
            internal virtual void Reset(FieldInfo fieldInfo)
            {
                ...
                this.fieldInfo = fieldInfo;
                InternedFieldName = fieldInfo.Name;
            ...
            public override SeekStatus SeekCeil(BytesRef term)
            {
                if (DEBUG_SURROGATES)
                {
                    System.Diagnostics.Debug.WriteLine("TE.seek target=" + UnicodeUtil.ToHexString(term.Utf8ToString()));
                }
                SkipNext = false;
                ...
                if (t != null && t.Field == InternedFieldName && term.BytesEquals(t.Bytes))
                {
                    ...
                    if (DEBUG_SURROGATES)
                    {
                        System.Diagnostics.Debug.WriteLine("  seek exact match");
                    }
                    Current = t.Bytes;
                ...
                else if (t == null || t.Field != InternedFieldName)
                {
                    ...
                    if (DEBUG_SURROGATES)
                    {
                        System.Diagnostics.Debug.WriteLine("  seek hit EOF");
                    }
                    ...
                    for (int i = ScratchTerm.Length - 1; i >= 0; i--)
                    {
                        if (IsHighBMPChar(ScratchTerm.Bytes, i))
                        {
                            if (DEBUG_SURROGATES)
                            {
                                System.Diagnostics.Debug.WriteLine("    found E pos=" + i + "; try seek");
                            }
                    ...
                    if (DEBUG_SURROGATES)
                    {
                        System.Diagnostics.Debug.WriteLine("  seek END");
                    }
                ...
                else
                {
                    ...
                    if (DEBUG_SURROGATES)
                    {
                        System.Diagnostics.Debug.WriteLine("  seek hit non-exact term=" + UnicodeUtil.ToHexString(t.Text()));
                    }
            ...
            internal virtual void SetNewSuffixStart(BytesRef br1, BytesRef br2)
            {
                ...
                for (int i = 0; i < limit; i++)
                {
                    ...
                    if (br1.Bytes[br1.Offset + i] != br2.Bytes[br2.Offset + i])
                    {
                        ...
                        if (DEBUG_SURROGATES)
                        {
                            System.Diagnostics.Debug.WriteLine("    set newSuffixStart=" + NewSuffixStart);
                        }
                        return;
                    }
                }
            ...
            public override BytesRef Next()
            {
                if (DEBUG_SURROGATES)
                {
                    System.Diagnostics.Debug.WriteLine("TE.next()");
                }
                if (SkipNext)
                {
                    if (DEBUG_SURROGATES)
                    {
                        System.Diagnostics.Debug.WriteLine("  skipNext=true");
                    }
                    SkipNext = false;
                ...
                if (TermEnum.Next() && TermEnum.Term().Field == InternedFieldName)
                {
                    ...
                    if (DEBUG_SURROGATES)
                    {
                        System.Diagnostics.Debug.WriteLine("  newSuffixStart=" + NewSuffixStart);
                    }
                    SurrogateDance();
                    Term t = TermEnum.Term();
                    if (t == null || t.Field != InternedFieldName)
                ...
                else
                {
                    ...
                    if (DEBUG_SURROGATES)
                    {
                        System.Diagnostics.Debug.WriteLine("  force cont");
                    }
