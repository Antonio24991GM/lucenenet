...
namespace Lucene.Net.Index
{
    ...
    public sealed class IndexUpgrader
    {
        ...
        {
            System.Diagnostics.Debug.WriteLine("Upgrades an index so all segments created with a previous Lucene version are rewritten.");
            System.Diagnostics.Debug.WriteLine("Usage:");
            System.Diagnostics.Debug.WriteLine("  java " + typeof(IndexUpgrader).Name + " [-delete-prior-commits] [-verbose] [-dir-impl X] indexDir");
            System.Diagnostics.Debug.WriteLine("this tool keeps only the last commit in an index; for this");
            System.Diagnostics.Debug.WriteLine("reason, if the incoming index has more than one commit, the tool");
            System.Diagnostics.Debug.WriteLine("refuses to run by default. Specify -delete-prior-commits to override");
            System.Diagnostics.Debug.WriteLine("this, allowing the tool to delete all but the last commit.");
            System.Diagnostics.Debug.WriteLine("Specify a " + typeof(FSDirectory).Name + " implementation through the -dir-impl option to force its use. If no package is specified the " + typeof(FSDirectory).Namespace + " package will be used.");
            System.Diagnostics.Debug.WriteLine("WARNING: this tool may reorder document IDs!");
        ...
        public static IndexUpgrader ParseArgs(string[] args)
        {
            ...
            while (i < args.Length)
            {
                ...
                {
                    //@out = System.Diagnostics.Contracts.
                }
                else if ("-dir-impl".Equals(arg))
                {
                    if (i == args.Length - 1)
                    {
                        System.Diagnostics.Debug.WriteLine("ERROR: missing value for -dir-impl option");
                        Application.Current.Exit();
                    }
                    i++;
                    dirImpl = args[i];
            ...
            if (dirImpl == null)
            {
                var task = ApplicationData.Current.LocalFolder.CreateFolderAsync(path, CreationCollisionOption.OpenIfExists).AsTask();
                task.Wait();
                dir = FSDirectory.Open(task.Result);
            ...
            else
            {
                var task = ApplicationData.Current.LocalFolder.CreateFolderAsync(path, CreationCollisionOption.OpenIfExists).AsTask();
                task.Wait();
                dir = CommandLineUtil.NewFSDirectory(dirImpl, task.Result);
            }
