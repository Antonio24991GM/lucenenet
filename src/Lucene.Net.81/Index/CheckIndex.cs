        ...
        public static void Main(string[] args)
        {
          ...
          while (i < args.Length)
          {
            ...
            else if (arg.Equals("-segment"))
            {
              if (i == args.Length - 1)
              {
                System.Diagnostics.Debug.WriteLine("ERROR: missing name for -segment option");
                Environment.Exit(1);
              }
              i++;
            ...
            else
            {
              if (indexPath != null)
              {
                System.Diagnostics.Debug.WriteLine("ERROR: unexpected extra argument '" + args[i] + "'");
                Environment.Exit(1);
              }
              indexPath = args[i];
            }
          ...
          {
            System.Diagnostics.Debug.WriteLine("ERROR: cannot specify both -fix and -segment");
            Environment.Exit(1);
          }
          ...
          {
            System.Diagnostics.Debug.WriteLine("ERROR: could not open directory \"" + indexPath + "\"; exiting");
            System.Diagnostics.Debug.Out.WriteLine(t.StackTrace);
            Environment.Exit(1);
          }

          CheckIndex checker = new CheckIndex(dir);
          checker.CrossCheckTermVectors = doCrossCheckTermVectors;
          checker.SetInfoStream(new StreamWriter(System.Diagnostics.Debug.OpenStandardOutput()), verbose);
          ...
          if (!result.Clean)
          {
            if (!doFix)
            {
              System.Diagnostics.Debug.WriteLine("WARNING: would write new segments file, and " + result.TotLoseDocCount + " documents would be lost, if -fix were specified\n");
            }
            else
            {
              System.Diagnostics.Debug.WriteLine("WARNING: " + result.TotLoseDocCount + " documents will be lost\n");
              ...
              System.Diagnostics.Debug.WriteLine("Wrote new segments file \"" + result.NewSegments.SegmentsFileName + "\"");
            }
          }
          System.Diagnostics.Debug.WriteLine("");
