...
namespace Lucene.Net.Store
{
    ...
    public class LockStressTest
    {
        ...
        public static void Main(string[] args)
        {
            //if (args.Length != 7)
            //{
            //    System.Diagnostics.Debug.WriteLine("Usage: java Lucene.Net.Store.LockStressTest myID verifierHost verifierPort lockFactoryClassName lockDirName sleepTimeMS count\n" + "\n" + "  myID = int from 0 .. 255 (should be unique for test process)\n" + "  verifierHost = hostname that LockVerifyServer is listening on\n" + "  verifierPort = port that LockVerifyServer is listening on\n" + "  lockFactoryClassName = primary LockFactory class that we will use\n" + "  lockDirName = path to the lock directory (only set for Simple/NativeFSLockFactory\n" + "  sleepTimeMS = milliseconds to pause betweeen each lock obtain/release\n" + "  count = number of locking tries\n" + "\n" + "You should run multiple instances of this process, each with its own\n" + "unique ID, and each pointing to the same lock directory, to verify\n" + "that locking is working correctly.\n" + "\n" + "Make sure you are first running LockVerifyServer.");
            //    Environment.Exit(1);
            //}
            ...
            //if (myID < 0 || myID > 255)
            //{
            //    System.Diagnostics.Debug.WriteLine("myID must be a unique int 0..255");
            //    Environment.Exit(1);
            //}
            ...
            //System.Diagnostics.Debug.WriteLine("Connecting to server " + addr + " and registering as client " + myID + "...");
            ...
            //System.Diagnostics.Debug.WriteLine("Finished " + count + " tries.");
        }
