    ...
    public class NRTCachingDirectory : Directory
    {
        ...
        public override void DeleteFile(string name)
        {
            lock (this)
            {
                if (VERBOSE)
                {
                    System.Diagnostics.Debug.WriteLine("nrtdir.deleteFile name=" + name);
                }
        ...
        public override IndexOutput CreateOutput(string name, IOContext context)
        {
            if (VERBOSE)
            {
                System.Diagnostics.Debug.WriteLine("nrtdir.createOutput name=" + name);
            }
            if (DoCacheWrite(name, context))
            {
                if (VERBOSE)
                {
                    System.Diagnostics.Debug.WriteLine("  to cache");
                }
                try
                {
                    @delegate.DeleteFile(name);
        ...
        public override void Sync(ICollection<string> fileNames)
        {
            if (VERBOSE)
            {
                System.Diagnostics.Debug.WriteLine("nrtdir.sync files=" + fileNames);
            }
        ...
        public override IndexInput OpenInput(string name, IOContext context)
        {
            lock (this)
            {
                if (VERBOSE)
                {
                    System.Diagnostics.Debug.WriteLine("nrtdir.openInput name=" + name);
                }
                if (Cache.FileExists(name))
                {
                    if (VERBOSE)
        ...
        private void UnCache(string fileName)
        {
            ...
            lock (UncacheLock)
            {
                if (VERBOSE)
                {
                    System.Diagnostics.Debug.WriteLine("nrtdir.unCache name=" + fileName);
                }
