...
namespace Lucene.Net.Store
{
    ...
    public class SimpleFSLockFactory : FSLockFactory
    {
        ...
        public SimpleFSLockFactory(StorageFolder lockDir)
        {
            LockDir = lockDir;
        }
        ...
        {
            CreateStorageFolder(lockDirName);
        }

        private async void CreateStorageFolder(string lockDirName)
        {
            
            LockDir = await ApplicationData.Current.LocalFolder.CreateFolderAsync(lockDirName);
        }
        ...
        public async override void ClearLock(string lockName)
        {
            if (LockDir_Renamed!=null)
            {
                if (LockPrefix_Renamed != null)
                {
                    lockName = LockPrefix_Renamed + "-" + lockName;
                }
                StorageFile lockFile = await LockDir_Renamed.CreateFileAsync(lockName, CreationCollisionOption.OpenIfExists);
                    //StorageFile(Path.Combine(LockDir_Renamed.FullName, lockName));
                try
                {
                ...
                {
                    if (lockFile!=null) // Delete failed and lockFile exists
    ...
    {
        internal StorageFile LockFile;
        internal StorageFolder LockDir;

        public SimpleFSLock(StorageFolder lockDir, string lockFileName)
        {
        ...
        private async void GetFile(string lockFileName)
        {
            LockFile = await LockDir.CreateFileAsync(lockFileName, CreationCollisionOption.OpenIfExists);
        }
        ...
        public async override void Release()
        {//LUCENE TO-DO
            try
            {
                if(LockFile!=null)
